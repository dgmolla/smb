...
const getMenuTool = new FunctionTool({
  name: "get_menu",
  description: "Get the list of available cookie flavors with prices. Call this when the user asks about the menu, what's available, or wants to see options.",
  execute: async () => {
    ...
    return {
      products: products.map(p => ({
        ...
        formatted: `${p.name} - ${symbol}${p.price.toFixed(2)}/${p.unit}`
      })),
      message: `We have ${products.length} delicious cookie flavors available.`
    };
  }
});
...
- get_menu: Show available cookie flavors and prices
