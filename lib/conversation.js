...
import { getProducts, getProductByNameAsync } from "./tools/data-access.js";
...
async function handleIdle(message, session, intent) {
  ...
  if (intent === "ORDER" || intent === "ORDER_ITEM") {
    ...
    if (extracted.items?.length > 0) {
      await addItemsToOrder(session, extracted.items);
      return {
 ...
 */
async function addItemsToOrder(session, items) {
