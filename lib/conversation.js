...
async function handleIdle(message, session, intent) {
  ...
  const aiResult = await processWithGemini(message, session.history.slice(-4), session.state);
  return {
...
async function handleConfirming(message, session, intent) {
  ...
  if (intent === "CONFIRM" || lower.includes("confirm")) {
    // Process the order
    ...
    return {
      response: `🎉 Order confirmed!\n\n` +
        ...
        `Thank you for your order, ${completedOrder.customerName}! We'll reach out to you at ${completedOrder.customerEmail} with pickup details.\n\n` +
