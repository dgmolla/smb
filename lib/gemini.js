 ...
 *
 * Using gemini-2.0-flash for better quality (~$0.50/month at 300 inquiries/week)
...
const SESSION_AI_LIMIT = 50; // Max AI calls per session
const sessionCallCounts = new Map();

// Daily usage monitoring
const DAILY_ALERT_THRESHOLD = parseInt(process.env.DAILY_ALERT_THRESHOLD || "300", 10);
const ALERT_EMAIL = process.env.ALERT_EMAIL || process.env.ESCALATION_EMAIL;
...
/**
 * Track daily AI calls and send alert if threshold exceeded
 */
async function trackDailyUsage() {
  ...
  // Send alert if threshold exceeded (only once per day)
  if (dailyCallCount >= DAILY_ALERT_THRESHOLD && !alertSentToday && ALERT_EMAIL) {
    alertSentToday = true;
    await sendUsageAlert(dailyCallCount);
  }
}
 ...
 */
async function sendUsageAlert(count) {
  try {
    ...
    await sendEmail({
      to: ALERT_EMAIL,
      subject: `⚠️ High AI Usage Alert - ${BUSINESS_CONFIG.businessName}`,
      html: `
        <h2>High Usage Alert</h2>
        <p>Your ${BUSINESS_CONFIG.businessName} chatbot has received <strong>${count} AI requests</strong> today.</p>
        <p>This exceeds the configured threshold of ${DAILY_ALERT_THRESHOLD} requests per day.</p>
        <p><strong>Date:</strong> ${new Date().toLocaleDateString()}</p>
        <p><strong>Time:</strong> ${new Date().toLocaleTimeString()}</p>
        <hr>
        <p><em>This alert is sent once per day when the threshold is exceeded.</em></p>
        <p>To adjust the threshold, set the DAILY_ALERT_THRESHOLD environment variable.</p>
      `,
      text: `High Usage Alert\n\nYour ${BUSINESS_CONFIG.businessName} chatbot has received ${count} AI requests today, exceeding the threshold of ${DAILY_ALERT_THRESHOLD}.\n\nDate: ${new Date().toLocaleDateString()}\nTime: ${new Date().toLocaleTimeString()}`
    });

    console.log(`Usage alert sent to ${ALERT_EMAIL}: ${count} requests today`);
  } catch (error) {
...
export function getDailyUsageStats() {
  const today = new Date().toDateString();
  if (today !== dailyCallDate) {
    return { count: 0, threshold: DAILY_ALERT_THRESHOLD, date: today, alertSent: false };
  }
  return {
...
export async function processWithGemini(message, history = [], currentState = "idle", sessionId = "default") {
  ...
  await trackDailyUsage(); // Track daily usage and alert if threshold exceeded
  console.log(`AI call for session ${sessionId}: ${limit.count + 1}/${SESSION_AI_LIMIT} (daily: ${dailyCallCount}/${DAILY_ALERT_THRESHOLD})`);

  const model = genAI.getGenerativeModel({
    model: "gemini-2.0-flash",  // Better quality, ~$0.50/month at 300 inquiries/week
    generationConfig: {
      maxOutputTokens: 500,
      temperature: 0.7,
    },
...
export async function extractOrderDetails(message) {
  ...
  const model = genAI.getGenerativeModel({
    model: "gemini-2.0-flash",  // Better quality for order extraction
    generationConfig: {
      maxOutputTokens: 200,
      temperature: 0.1,
    },
...
IMPORTANT: Quantity is in DOZENS, not individual cookies.
- "1 dozen chocolate chip" → quantity: 1
- "2 dozen sugar" → quantity: 2
- "chocolate chip" (no number) → quantity: 1

Return JSON only:
{"items": [{"flavor": "exact flavor name from list above", "quantity": number_of_dozens}]}

If no valid items match, return {"items": []}`;
...
export function classifyIntentQuick(message) {
  ...
  // Check for just a number (likely order quantity) - e.g., "1", "2", "3"
  if (/^\d+$/.test(lower)) {
    return "ORDER_ITEM";
  }
  ...
  if (/^(yes|yeah|yea|confirm|correct|sure|ok|okay)[\s!.]*$/i.test(lower)) {
    return "CONFIRM";
  }
  if (/^(no|nope|cancel|nevermind|stop)[\s!.]*$/i.test(lower)) {
    return "CANCEL";
  }
