...
import { BUSINESS_CONFIG, getAvailableProducts, searchKnowledge } from "../config.js";
...
/**
 * Try to answer FAQ locally using keyword matching
...
export async function tryLocalFAQ(message) {
  ...
  // Pattern: "ingredients of X", "what's in X", "X ingredients"
  if (lower.includes("ingredient") || lower.includes("what's in") || lower.includes("whats in") || lower.includes("made of") || lower.includes("made with")) {
    ...
    for (const product of products) {
      ...
      // Check product name and common aliases
      const aliases = Object.entries(BUSINESS_CONFIG.flavorAliases)
        .filter(([_, name]) => name === product.name)
        .map(([alias, _]) => alias.toLowerCase());

      const allNames = [productLower, ...aliases];
  ...
  if (results.length > 0 && results[0].score >= 3) {
    console.log("FAQ answered locally:", results[0].question);
    return results[0].answer;
  }

  // Check for common patterns not in knowledge base
...
export function classifyIntentQuick(message) {
  ...
  if (/(order|buy|want|get|need|like)\s*(some|a|an|the|to)?\s*(order|cookie|dozen)?/i.test(lower)) {
    // Check if there's a flavor mentioned - if so it's ORDER_ITEM
    const flavorPatterns = [
      "froot\\s*loop", "fruit\\s*loop", "matcha", "oreo", "lotus", "biscoff",
      "corn\\s*flake", "cheddar", "cheese", "s'?more", "dubai", "chewy"
    ].join("|");
    if (new RegExp(flavorPatterns, "i").test(lower)) {
      return "ORDER_ITEM";
    }
    return "ORDER";
  }
